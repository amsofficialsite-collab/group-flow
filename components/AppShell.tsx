...
import { BarChart3, CalendarDays, FileText, History, LayoutDashboard, Settings, Users } from "lucide-react";
...
export default function AppShell({ title, children }: { title: string; children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen md:grid md:grid-cols-[250px_1fr]">
      ...
      <main>
        <header className="flex items-center justify-between border-b border-white/10 px-6 py-5 md:px-8"><div><p className="text-xs uppercase tracking-[0.2em] text-cyan-300">GROUP FLOW</p><h1 className="text-2xl font-bold">{title}</h1></div><div className="rounded-full border border-white/10 px-3 py-1.5 text-sm text-white/60">V0.1</div></header>

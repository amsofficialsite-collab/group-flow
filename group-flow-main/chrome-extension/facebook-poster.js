(() => {
  ...
  function showPanel(job) {
    ...
    panel.innerHTML = `
      <div style="font-weight:700;font-size:17px">GROUP FLOW Posting Agent V12</div>
    ...
    panel.querySelector("#gf-fail").onclick = () => {
      void finishJob({ result: "failed", notes: "ผู้ใช้ยกเลิกจาก Posting Agent" }).catch(console.error);
      panel.remove();
    };
    return panel;
  }
  ...
  function finishJob(message) {
    return new Promise((resolve, reject) => {
      chrome.runtime.sendMessage({ type: "GROUPFLOW_FINISH_JOB", ...message }, (response) => {
        if (chrome.runtime.lastError) {
          reject(new Error(chrome.runtime.lastError.message));
          return;
        }
        if (!response?.ok) {
          reject(new Error(response?.error || "บันทึกผลกลับ GROUP FLOW ไม่สำเร็จ"));
          return;
        }
        resolve(response);
      });
  ...
  async function run(job) {
    ...
    try {
      ...
      if (job.autoPost) {
        ...
        postButton.click();
        await sleep(4000);
        await finishJob({ result: "posted", postUrl: location.href, notes: "โพสต์อัตโนมัติจาก GROUP FLOW Posting Agent V12" });
        setStatus("โพสต์สำเร็จ และพร้อมทำคิวถัดไปแล้ว");
      } else {
        ...
        manualPost.onclick = async () => {
          ...
          await sleep(4000);
          await finishJob({ result: "posted", postUrl: location.href, notes: "ผู้ใช้ตรวจสอบและกดโพสต์ผ่าน Posting Agent V12" });
        };
      }
    } catch (error) {
      setStatus(`หยุดทำงาน: ${error?.message || String(error)}`);
      void finishJob({ result: "failed", notes: error?.message || String(error) }).catch(console.error);
    }
  }
